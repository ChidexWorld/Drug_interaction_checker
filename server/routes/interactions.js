 ...
 *   post:
 *     summary: Check interactions between multiple drugs
 *     tags: [Interactions]
 *     requestBody:
 *       required: true
 *       content:
  ...
  try {
        ...
        if (interaction) {
          ...
          if (condition_id) {
            ...
            if (conditionAdjustment) {
              finalInteraction.severity_score =
                conditionAdjustment.adjusted_severity_score;
              finalInteraction.interaction_type =
                conditionAdjustment.adjusted_interaction_type;
              finalInteraction.condition_note =
                conditionAdjustment.condition_specific_note;
    ...
    const maxSeverity =
      interactions.length > 0
        ? Math.max(...interactions.map((i) => i.severity_score))
        : 0;
